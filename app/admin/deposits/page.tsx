...
export default function DepositsPage() {
  ...
  async function handleApproval(txId: string, approved: boolean) {
    ...
    try {
      ...
      if (res.ok) {
        setDeposits(deposits.filter((d) => d.id !== txId));
      } else {
    ...
    <div className="space-y-6">
      ...
      {deposits.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-8 text-center">
          <p className="text-gray-600 dark:text-gray-400">No pending deposits</p>
        </div>
      ) : (
                ...
                {deposits.map((deposit) => (
                  <tr key={deposit.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <td className="px-6 py-4">
                      <div>
                        <p className="font-medium text-gray-900 dark:text-white">{deposit.user.name}</p>
                        <p className="text-sm text-gray-600 dark:text-gray-400">{deposit.user.email}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                      ${deposit.amount.toFixed(2)}
                    </td>
                    <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{deposit.method}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleApproval(deposit.id, true)}
                        ...
                        <button
                          onClick={() => handleApproval(deposit.id, false)}
                          disabled={processingId === deposit.id}
