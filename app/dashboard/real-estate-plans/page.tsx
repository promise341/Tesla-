...
import {
  Home, ChevronRight, TrendingUp, DollarSign, Shield,
  Building2, MapPin, Flag, BarChart2, Layers,
  Info, CheckCircle2, AlertCircle, Loader2, X, Lock,
} from "lucide-react";
...
const REAL_ESTATE_PLANS = [
  {
    ...
    bonus: 20,
    type: "Residential",
    // Suburban single-family home — warm, welcoming residential property
  ...
  },
  {
    ...
    bonus: 50,
    type: "Commercial",
    // Modern glass office building — professional commercial real estate
  ...
  },
  {
    ...
    bonus: 150,
    type: "Mixed-Use",
    // Urban mixed-use development — retail street level with apartments above
  ...
  },
  {
    ...
    bonus: 300,
    type: "Industrial",
    // Large industrial warehouse / logistics facility
  ...
  },
  {
    ...
    bonus: 500,
    type: "REITs",
    // City skyline with multiple high-rise buildings — REIT diversified portfolio
  ...
  },
  {
    ...
    bonus: 1000,
    type: "Luxury",
    // Stunning luxury villa with pool — high-end estate property
    img: "https://images.unsplash.com/photo-1613977257592-4871e5fcd7c4?w=600&q=80",
  },
];

type Plan = (typeof REAL_ESTATE_PLANS)[number];

const PORTFOLIO_SECTORS = [
  { icon: <Building2 size={22} className="text-primary-500" />, bg: "bg-red-50 dark:bg-red-900/20", label: "Residential" },
  { icon: <Building2 size={22} className="text-blue-500" />, bg: "bg-blue-50 dark:bg-blue-900/20", label: "Commercial" },
  { icon: <MapPin size={22} className="text-orange-500" />, bg: "bg-orange-50 dark:bg-orange-900/20", label: "Industrial" },
  { icon: <Flag size={22} className="text-yellow-500" />, bg: "bg-yellow-50 dark:bg-yellow-900/20", label: "Retail" },
  { icon: <Layers size={22} className="text-purple-500" />, bg: "bg-purple-50 dark:bg-purple-900/20", label: "Mixed-Use" },
  { icon: <BarChart2 size={22} className="text-green-500" />, bg: "bg-green-50 dark:bg-green-900/20", label: "REITs" },
];

/* ── Confirm Modal ── */
function ConfirmModal({
  plan, amount, onConfirm, onClose, loading, error,
}: {
  plan: Plan; amount: number; onConfirm: () => void;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-md p-6">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="font-extrabold text-gray-900 dark:text-white">Confirm Real Estate Investment</h3>
            <p className="text-xs text-gray-400">{plan.name}</p>
          </div>
        </div>
        <div className="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-4 space-y-2 mb-5">
          {[
            { label: "Plan", value: plan.name, red: false },
            { label: "Amount", value: `$${amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, red: false },
            { label: "Property Type", value: plan.type, red: false },
            { label: "Return Rate", value: plan.rateLabel, red: true },
            { label: "Duration", value: `${plan.duration} Days`, red: false },
            { label: "Daily Return", value: `$${daily.toFixed(2)}`, red: true },
            { label: `Total (${plan.duration}d)`, value: `$${total.toFixed(2)}`, red: true },
          ].map((r) => (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-md p-8 text-center">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">
          You joined <span className="font-bold text-gray-900 dark:text-white">{plan.name}</span>
        </p>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
          <span className="font-bold text-primary-500">${amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span> invested successfully.
        </p>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden flex flex-col">
      ...
      <div className="relative h-28 overflow-hidden">
        ...
        {plan.tag && (
          <span className={`absolute top-2 right-2 ${plan.tagColor} text-white text-[10px] font-black px-2 py-0.5 rounded-full`}>
            → {plan.tag}
          </span>
        )}
        <div className="absolute bottom-2 left-3">
          <div className="inline-flex items-center gap-1.5 bg-black/30 backdrop-blur-sm rounded-lg px-2 py-1">
            <span className="text-sm font-black text-white">{plan.roi}</span>
            <span className="text-[10px] text-white/80">{plan.roiLabel}</span>
          </div>
          <p className="text-[10px] text-white/80 font-semibold mt-0.5">Real Estate · {plan.type}</p>
        </div>
      </div>
      ...
      <div className="p-5 flex flex-col flex-1 space-y-3">
        <div className="flex items-start justify-between">
          ...
          <div className="text-right">
            ...
            <p className="text-xs font-bold text-gray-700 dark:text-gray-300">{plan.duration} Days</p>
          </div>
        </div>

        <p className="text-xl font-black text-primary-500">
          ${plan.min.toLocaleString()} <span className="text-sm font-semibold text-gray-400">minimum</span>
        </p>

        <div className="space-y-1.5">
          {[
            { label: "Investment Range", value: `$${plan.min.toLocaleString()} – $${plan.max.toLocaleString()}`, red: false, blue: false },
            { label: "Return Rate", value: plan.rateLabel, red: true, blue: false },
            { label: "Welcome Bonus", value: `$${plan.bonus}`, red: false, blue: true },
          ].map((r) => (
            <div key={r.label} className="flex items-center justify-between bg-gray-50 dark:bg-gray-900/40 rounded-lg px-3 py-1.5 text-xs">
              <span className="text-gray-500 dark:text-gray-400">{r.label}</span>
              <span className={r.red ? "font-bold text-primary-500" : r.blue ? "font-bold text-blue-500" : "font-bold text-gray-700 dark:text-gray-300"}>
                {r.value}
              </span>
            </div>
          ))}
        ...
        <div className="flex justify-between text-[10px] text-gray-400">
          ...
          <span>${plan.max.toLocaleString()}</span>
        </div>

        <div className="bg-red-50 dark:bg-red-900/10 rounded-xl px-4 py-2.5 space-y-1">
          <div className="flex justify-between text-xs">
            <span className="text-gray-500 dark:text-gray-400">Daily Return:</span>
            <span className="font-bold text-primary-500">${daily.toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="text-gray-500 dark:text-gray-400">Total Return ({plan.duration} days):</span>
        ...
        <button
          ...
          className="mt-auto w-full flex items-center justify-center gap-2 py-3 bg-primary-500 hover:bg-primary-600 text-white font-extrabold text-sm rounded-xl transition-colors shadow-md shadow-primary-500/20"
        >
          <Building2 size={15} /> Invest in Real Estate
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-lg p-6">
        ...
        <div className="flex items-center gap-3 mb-5">
          <div className="w-10 h-10 rounded-xl bg-primary-50 dark:bg-primary-900/20 flex items-center justify-center"><Building2 size={20} className="text-primary-500" /></div>
...
export default function RealEstatePlansPage() {
  ...
  const [userPlanNames, setUserPlanNames] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/user/me")
      .then((r) => r.json())
      .then((d) => { if (d?.balance !== undefined) setUserBalance(Number(d.balance)); })
      .catch(() => {});

    // Check if user has access to real estate plans
    fetch("/api/user/real-estate-plans")
      .then((r) => r.json())
      .then((data) => {
        setHasAccess(data.hasAccess || false);
        if (data.plans) {
          setUserPlanNames(data.plans.map((p: any) => p.planName));
        }
      })
      .catch(() => setHasAccess(false));

    // Check general plans subscription status
    fetch("/api/plans")
      .then((r) => r.json())
      .then((plans) => {
  ...
  async function handleConfirm() {
    ...
    try {
      const res = await fetch("/api/plans/subscribe", {
        ...
        body: JSON.stringify({ planName: confirmPlan.name, amount: confirmAmount, rate: confirmPlan.rate, duration: confirmPlan.duration }),
      });
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4 max-w-md mx-auto text-center">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-8 leading-relaxed">
          The Real Estate investment portal is locked. Please subscribe to at least one basic investment plan under the "All Plans" section first to unlock access.
        </p>
        <Link
    ...
    <>
      ...
      <div className="space-y-6 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-1.5 text-xs text-gray-400">
          ...
          <ChevronRight size={11} />
          <span className="text-gray-600 dark:text-gray-300 font-semibold">Real Estate</span>
        </div>
        ...
        {hasAccess === false && (
          <div className="flex flex-col items-center justify-center py-20 px-4">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-center max-w-md mb-8">
              Real estate investment plans are exclusive. Purchase a plan to unlock access to our premium property portfolio.
            </p>
            <Link
              href="/dashboard/real-estate-plans/request-access"
              className="inline-flex items-center gap-2 px-6 py-3 bg-primary-500 hover:bg-primary-600 text-white font-bold rounded-xl transition-colors shadow-md"
            >
              <Lock size={16} />
              Request Plan Access
            </Link>
          ...
          <>
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-2xl lg:text-3xl font-extrabold text-primary-500">Real Estate Investment Plans</h1>
              </div>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">Build wealth through strategic real estate investments with proven returns</p>
              <div className="flex flex-wrap gap-2">
                {[
                  { label: "Stable Returns", cls: "bg-red-50 dark:bg-red-900/20 text-primary-500 border-primary-200 dark:border-primary-800" },
                  { label: "Diversified Portfolio", cls: "bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-800" },
                  { label: "Secure Investments", cls: "bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 border-green-200 dark:border-green-800" },
                ].map((b) => (
                  <span key={b.label} className={`inline-flex items-center gap-1 text-xs font-bold px-3 py-1 rounded-full border ${b.cls}`}>
            ...
            <div className="text-right bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 px-5 py-3 shadow-sm flex-shrink-0">
              <p className="text-2xl font-black text-primary-500">4-20%</p>
        ...
        <div className="bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-extrabold text-gray-900 dark:text-white">Real Estate Market Insights</h3>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
            {[
              { value: "$2.4T", label: "Global Market Size", color: "text-primary-500" },
              { value: "8.2%", label: "Avg. Annual Growth", color: "text-blue-600 dark:text-blue-400" },
              { value: "15+", label: "Property Types", color: "text-purple-600 dark:text-purple-400" },
              { value: "50+", label: "Global Markets", color: "text-green-600 dark:text-green-400" },
            ].map((s) => (
              <div key={s.label}>
        ...
        {/* Plans Grid - Only show purchased plans */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {REAL_ESTATE_PLANS.filter(plan => userPlanNames.includes(plan.name)).map((plan) => (
            <PlanCard key={plan.name} plan={plan} onInvest={handleInvest} />
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 className="font-extrabold text-gray-900 dark:text-white">Why Choose Real Estate Investment?</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            {[
              { icon: <TrendingUp size={20} className="text-primary-500" />, bg: "bg-red-50 dark:bg-red-900/20", title: "Stable Returns", desc: "Real estate typically provides consistent, predictable returns with lower volatility than other investments." },
              { icon: <Shield size={20} className="text-blue-500" />, bg: "bg-blue-50 dark:bg-blue-900/20", title: "Inflation Protection", desc: "Property values and rental income typically increase with inflation, protecting your purchasing power over time." },
              { icon: <DollarSign size={20} className="text-green-500" />, bg: "bg-green-50 dark:bg-green-900/20", title: "Passive Income", desc: "Generate regular rental income without active management, creating a reliable passive income stream." },
            ].map((f) => (
              <div key={f.title} className="flex items-start gap-3">
                <div className={`w-10 h-10 rounded-full ${f.bg} flex items-center justify-center flex-shrink-0`}>{f.icon}</div>
                <div>
                  <h4 className="font-extrabold text-sm text-gray-900 dark:text-white mb-1">{f.title}</h4>
        ...
        {/* Portfolio Sectors */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
          <h3 className="font-extrabold text-gray-900 dark:text-white text-center mb-6">Our Real Estate Portfolio</h3>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 className="font-extrabold text-gray-900 dark:text-white">Investment Strategy & Risk Management</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              <ul className="space-y-2 text-xs text-gray-500 dark:text-gray-400">
                {[
                  ...
                  "Focus on high-yield residential and commercial assets",
            ...
            <div>
              <p className="text-xs font-extrabold text-gray-700 dark:text-gray-300 mb-3">Investment Benefits:</p>
              <ul className="space-y-2 text-xs text-gray-500 dark:text-gray-400">
                {[
                  "Regular income distributions from rental returns",
                  "Long-term capital appreciation potential",
                  "Inflation-hedged asset class",
                  "Backed by tangible, physical assets",
                ].map((item) => (
                  <li key={item} className="flex items-center gap-2">
